...
async def scrape_skymovies(html_content, client, skip_already_sent=True):
    """
    ...
    if not movie_divs:
        # If detail page directly
        info = await scrape_skymovies_detail_async(html_content, client, skip_already_sent)
        if info:
            movies.append(info)
        return movies
    ...
    for div in movie_divs:
        ...
        if clean_href.startswith("http"):
            ...
            detail_url = f"{SKYMOVIES_URL.rstrip('/')}/{clean_href.lstrip('/')}"

        if skip_already_sent:
            is_sent = await already_sent(detail_url)
            if is_sent:
        ...
        # Send to telegram and mark as sent
        await send_movie_with_expanded_hosts(client, movie_info)
...
async def scrape_skymovies_detail_async(detail_html, client, skip_already_sent=True):
    """
    ...
    # Note: detail_url might be empty here since we are just parsing the HTML directly without the link
    detail_url = ""

    if skip_already_sent and detail_url:
        is_sent = await already_sent(detail_url)
        if is_sent:
            return None
...
def extract_host_links_from_howblogs(howblogs_url):
    ...
    try:
        scraper = cloudscraper.create_scraper()
        resp = scraper.get(howblogs_url, timeout=15)
        resp.raise_for_status()
...
async def send_movie_with_expanded_hosts(client, movie_info):
    ...
    if SKYMOVIES_CHANNEL_ID:
        try:
            ...
            print(f"✅ Sent to Skymovies Channel: {movie_info.get('movie_title')}")
        except Exception as e:
            print(f"⚠️ Skymovies send failed: {e}")
    else:
        print("⚠️ SKYMOVIES_CHANNEL_ID is not set, skipping message send.")

async def start_skymovies_scraper(client):
    print("🔍 Scraping Skymovies...")
    html = await asyncio.to_thread(fetch_html, SKYMOVIES_URL)
    if html:
        await scrape_skymovies(html, client)
